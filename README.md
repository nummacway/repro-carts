...
- `$7002.5`: Use the _last_ 8 KiB of the corresponding 32 KiB of SRAM (see below) instead of four banks. So it's like writing `$03` to `$4000` and then locking that register. If bit 5 and 6 are both set, RAM will be unavailable. The latter constellation is used by the 61-in-1 and 108-in-1 menu, while the others only set bit 6 if they don't want RAM. The actual 8 KiB mode is only found in the 61-in-1 menu where it is used for Super Mario Land 2 and 3. It is not used to increase the number of RAM-enabled games, even though Super Mario Bros. Deluxe would qualify to share its RAM with one of these two, but there are only 7 games with a save feature on that cart anyway. If `$7002.5` is clear, memory at `$4000-$7fff` will be forced into MBC5 mode. This means that if MBC1 mode is enabled and this bit is clear, memory at `$0000-$3fff` will be in MBC1 mode (e.g. it will be determined by up to two bits from the `$4000-$5fff` register (which doesn't even make sense with 32 KiB of RAM), meaning you can map up to 4 different banks there) but `$4000-$7fff` will be in MBC5 mode (meaning you can map up to 512 different banks there). This makes no sense. I consider this a bug.
- `$7002.6`: SRAM disable.
